...
각 도메인 패키지는 계층별로 하위 폴더를 나눠서 구성합니다:

```
domain/user/
├── entity/                 # Entity + 관련 Enum (테이블 매핑)
│   ├── User.java
│   └── Role.java, TravelStyle.java, UserStatus.java
├── repository/              # JPA Repository
│   └── UserRepository.java
├── service/                  # 비즈니스 로직 (아직 없는 도메인도 있음)
│   └── UserService.java
├── controller/                # API 엔드포인트 (아직 없는 도메인도 있음)
│   └── UserController.java
└── dto/
    ├── SignupRequest.java
    └── UserResponse.java
...
`controller/`, `service/`는 API가 실제로 필요한 도메인부터 만들어졌고(`user`, `verification`, `auth`, `city`, `main`, `admin`), 나머지 도메인은 아직 `entity/`, `repository/`까지만 있습니다. **각 담당자가 해당 도메인 폴더 안에 `service/`, `controller/`를 이어서 채우면 됩니다.**
