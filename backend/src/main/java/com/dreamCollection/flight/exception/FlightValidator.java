...
public class FlightValidator {
    ...
    public void validateReturnSearch(FlightReturnRequestDTO request) {
        if (request == null) {
            throw new FlightException("오는편 검색 요청 정보가 없습니다.");
        }
        ...
        if (request.getArrivalAirportCode() == null || request.getArrivalAirportCode().isBlank()) {
            throw new FlightException("도착 공항 코드가 없습니다.");
        }

        if (request.getDepartureToken() == null || request.getDepartureToken().isBlank()) {
            throw new FlightException("가는편 선택 토큰이 없습니다.");
        }
    }
}