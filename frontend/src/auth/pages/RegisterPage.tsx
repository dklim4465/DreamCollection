...
// 비밀번호 강도를 간단히 3단계로 평가 (길이 / 대소문자 혼용 / 숫자+특수문자)
function getPasswordStrength(pw: string) {
  ...
  const tiers = [
    { label: "8자 이상 입력해주세요", color: "rgb(var(--color-error))", width: "20%" },
    { label: "조금 더 안전하게 만들 수 있어요", color: "rgb(var(--color-error))", width: "45%" },
    { label: "적당한 강도예요", color: "rgb(var(--color-tertiary))", width: "72%" },
    { label: "안전한 비밀번호예요", color: "rgb(var(--color-primary))", width: "100%" },
  ] as const;
...
export default function RegisterPage() {
  ...
  return (
    <AuthLayout
      ...
      title="당신의 다음 여정을 기록해요"
      description="일정, 동행, 그리고 다녀온 나라의 도장까지 — 여행의 모든 순간을 한 곳에 모아보세요."
      highlight={{ value: "47", label: "개국의 여행 도장을 모을 수 있어요" }}
    >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-2">
              <span className="auth-section-badge">1</span>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {emailIsChecked && emailDupStatus === "available" && (
                <span className="chip-primary w-fit flex items-center gap-1">
                  <span className="material-symbols-outlined text-base">check_circle</span>
                  사용 가능한 이메일이에요
                </span>
              )}
              {emailIsChecked && emailDupStatus === "duplicate" && (
                <span className="chip-error w-fit flex items-center gap-1">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {!errors.password && passwordStrength && (
                <div>
                  ...
                  <p className="text-label-sm mt-1" style={{ color: passwordStrength.color }}>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {!errors.passwordConfirm && passwordConfirmValue && passwordConfirmValue === password && (
                <span className="chip-primary w-fit flex items-center gap-1">
                  <span className="material-symbols-outlined text-base">check_circle</span>
                  비밀번호가 일치해요
                </span>
          ...
          <div className="flex flex-col gap-2 pt-4 border-t border-outline-variant">
            <div className="flex items-center gap-2 mb-1">
              <span className="auth-section-badge">2</span>
              <span className="text-label-sm font-bold uppercase tracking-wider text-on-surface-variant">
                본인 인증
              </span>
            </div>
            ...
            <div className="flex gap-2 rounded-xl bg-surface-container-low p-1">
              <button
                ...
                className={
                  method === "EMAIL"
                    ? "flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-surface-container-lowest py-2.5 text-center font-bold text-label-md traveler-glow"
                    : "flex-1 flex items-center justify-center gap-1.5 rounded-lg py-2.5 text-center text-on-surface-variant text-label-md"
                }
              >
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="input-base w-full pl-10 opacity-70"
                    />
                  </div>
                  <button
                    type="button"
                ...
                {emailCodeStatus === "sent" && (
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="auth-timer-track flex-1">
                        <div
                          className="auth-timer-fill"
                          style={{
                            width: `${(emailTimeLeft / CODE_TTL_SECONDS) * 100}%`,
                            backgroundColor:
                              emailTimeLeft <= 60 ? "rgb(var(--color-error))" : "rgb(var(--color-primary))",
                          }}
                        />
                      </div>
                      <span className="text-label-sm font-bold text-error whitespace-nowrap">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="auth-timer-track flex-1">
                        <div
                          className="auth-timer-fill"
                          style={{
                            width: `${(phoneTimeLeft / CODE_TTL_SECONDS) * 100}%`,
                            backgroundColor:
                              phoneTimeLeft <= 60 ? "rgb(var(--color-error))" : "rgb(var(--color-primary))",
                          }}
                        />
                      </div>
                      <span className="text-label-sm font-bold text-error whitespace-nowrap">
                        {formatTime(phoneTimeLeft)}
                      </span>
                    </div>
                  </div>
                )}
          ...
          <div className="flex flex-col gap-4 pt-4 border-t border-outline-variant">
            <div className="flex items-center gap-2">
              <span className="auth-section-badge">3</span>
              <span className="text-label-sm font-bold uppercase tracking-wider text-on-surface-variant">
                프로필
              </span>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <span className="material-symbols-outlined pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-lg text-on-surface-variant">
                  expand_more
                </span>
              </div>
            </div>
